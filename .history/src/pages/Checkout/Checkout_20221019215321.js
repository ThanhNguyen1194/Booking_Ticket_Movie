    ...
    const operations = <Fragment>
        {!_.isEmpty(userLogin) ? <Fragment>
            <button onClick={() => {
                ...
                <div style={{ width: 50, height: 50, display: 'inline-block', justifyContent: 'center', alignItems: 'center', lineHeight: 1.8 }} className="text-2xl ml-5 rounded-full bg-red-200">{userLogin.name.substr(0, 1)}</div>{"  "}Hello ! {userLogin.name}</button>
            <button onClick={() => {
