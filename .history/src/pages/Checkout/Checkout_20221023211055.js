...
export default function CheckoutTab(props) {
    ...
    const operations = <Fragment>
        {!_.isEmpty(userLogin) ? <Fragment>
            <button className='text-lg' onClick={() => {
                ...
                <span style={{ width: 50, height: 50, display: 'inline-block', justifyContent: 'center', alignItems: 'center', lineHeight: 1.8 }} className="text-2xl ml-5 mr-2 rounded-full bg-red-200">{userLogin.taiKhoan?.substr(0, 1)}</span>  Hello ! {userLogin?.hoTen}</button> {console.log(userLogin)}
            <button onClick={() => {
    ...
    return <div className="p-5">
        ...
        <Tabs tabBarExtraContent={operations} activeKey={tabActive} onChange={(key) => {
            ...
            <TabPane tab={<div className="text-center" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}><NavLink to="/"><HomeOutlined style={{ marginLeft: 10, fontSize: 25 }} /></NavLink></div>} key="3"></TabPane>
        </Tabs>
